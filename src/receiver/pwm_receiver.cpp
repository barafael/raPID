...
/*@ requires valid_access: \valid(_offsets + (0 .. NUM_CHANNELS - 1));
    ...
    ensures RXTX_init: ghost_pwmreceiver_status == PWM_RECEIVER_INITIALIZED;
*/
void pwm_receiver_init(uint8_t _throttle_pin,
    ...
    //implements: RXTX_register_interrupts
...
/*@
   requires RXTX_init: ghost_pwmreceiver_status == PWM_RECEIVER_INITIALIZED;
   ...
   ensures GLOBAL_interrupt_reenable: RXTX_interrupt_reenable: ghost_interrupt_status == INTERRUPTS_ON;
*/
const void receiver_update(int16_t channels[NUM_CHANNELS]) {
    ...
    */
    ...
    mock_interrupts();
    //@ assert GLOBAL_interrupt_reenable: RXTX_interrupt_reenable: ghost_interrupt_status == INTERRUPTS_ON;
...
pre_inversion:
    ...
    */
    ...
    //@ assert RXTX_bounded_results: 1000 <= channels[1] <= 2000;
    //@ assert RXTX_bounded_results: 1000 <= channels[2] <= 2000;
    //@ assert RXTX_bounded_results: 1000 <= channels[3] <= 2000;
    //@ assert RXTX_bounded_results: 1000 <= channels[4] <= 2000;
    //@ assert RXTX_bounded_results: 1000 <= channels[5] <= 2000;
    ...
    */
    for (size_t index = 0; index < NUM_CHANNELS; index++) {
        ...
        /*@ assert GLOBAL_undef_behavior: mem_access: \valid_read(channels + index); */
        /*@ assert GLOBAL_undef_behavior: index_bound: index < 6; */
        /*@ assert GLOBAL_undef_behavior: signed_overflow: -2147483648 ≤ (int)*(channels + index) + (int)pwm_offsets[index]; */
        /*@ assert GLOBAL_undef_behavior: signed_overflow: (int)*(channels + index) +(int)pwm_offsets[index] ≤ 2147483647; */
    ...
    */
    for (size_t index = 0; index < NUM_CHANNELS; index++) {
      ...
      /*@ assert GLOBAL_undef_behavior: mem_access: \valid_read(channels + index); */
      /*@ assert GLOBAL_undef_behavior: index_bound: index < 6; */
      /*@ assert GLOBAL_undef_behavior: signed_overflow: -2147483648 ≤ (int)*(channels + index) +(int)trims[index]; */
      /*@ assert GLOBAL_undef_behavior: signed_overflow: (int)*(channels + index) + (int)trims[index] ≤ 2147483647; */
...
/*@
   ...
   behavior has_connection:
     ...
     ensures \result == true;
   behavior RXTX_connection_timeout:
     assumes \exists integer i; 0 <= i < NUM_CHANNELS ==> pwm_pulse_duration_shared[i] > 80000;
     ensures ghost_interrupt_status == INTERRUPTS_ON;
     ensures \result == false;
   
   complete behaviors has_connection, RXTX_connection_timeout;
   disjoint behaviors has_connection, RXTX_connection_timeout;
*/
const bool has_signal() {
    ...
    */
    for (size_t index = 0; index < NUM_CHANNELS; index++) {
        ...
        //implements: RXTX_connection_timeout
...
/*@ requires RXTX_interrupt_reenable: ghost_interrupt_status == INTERRUPTS_ON;
   requires RXTX_init: ghost_pwmreceiver_status == PWM_RECEIVER_INITIALIZED;
...
void update_throttle() {
    if (mock_digitalRead(throttle_pin) == HIGH) {
        ...
        //implements: GLOBAL_elapsed_time, GLOBAL_elapsed_timeCalculation
