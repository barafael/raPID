...
class FastPWMOutput : Output {
    ...
    public:
    FastPWMOutput(uint8_t pin, float throttle_volume, float roll_volume, float pitch_volume, float yaw_volume);

    void apply(uint16_t _milli_throttle, float roll_stbl, float pitch_stbl, float yaw_stbl) override;
